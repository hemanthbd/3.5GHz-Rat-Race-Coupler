...
void processBuffer(void)
{
    ...
    if (pingPong == PING)
    {
		...
		{
		xL[2*i] = gBufferRcvPingL[i];
		xL[2*i+1]=0;
		}
		
	    for(i=0;i<BUFFSIZE/2;i++)
		{
		xR[2*i] = gBufferRcvPingR[i];
		xR[2*i+1]=0;
		}
        if (!switch3) {
			...
			spectrum[0] = 32700; 
		    copyData(spectrum, gBufferXmtPingL, BUFFSIZE/2);
			...
			copyData(spectrum, gBufferXmtPingR, BUFFSIZE/2);
        ...
        if (!switch3) {
            ...
            for(i=0;i<BUFFSIZE/2;i++)
			{
			xL[2*i] = gBufferRcvPongL[i];
			xL[2*i+1]=0;
			}
			
		    for(i=0;i<BUFFSIZE/2;i++)
			{
			xR[2*i] = gBufferRcvPongR[i];
			xR[2*i+1]=0;
			}
			...
			spectrum[i] = xL[2*i]*xL[2*i]+xL[2*i+1]*xL[2*i+1];
			spectrum[0] = 32700;
			copyData(spectrum, gBufferXmtPongL, BUFFSIZE/2);
			...
			spectrum[i] = xR[2*i]*xR[2*i]+xR[2*i+1]*xR[2*i+1];
			spectrum[0] = 32700;
			copyData(spectrum, gBufferXmtPongR, BUFFSIZE/2);    
            // Process samples from left channel, then right channel
